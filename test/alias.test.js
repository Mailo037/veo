...
test('POSIX alias executes veo with literal arguments and alias remove deletes it', { skip: process.platform === 'win32' }, async () => {
  ...
  try {
    ...
    assert.equal(await aliasMain(['remove', 'vo', '--bin-dir', binDir], { stdout: removed }), 0);
